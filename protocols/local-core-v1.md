...
calls the SPARK capability authority on the attenuated child. Refusals carry
a typed `decision` (`REFUSED_CAPABILITY` or `REFUSED_APPROVAL`) plus a
`reason` (`no_parent_grant`, `grant_corrupt`, `parent_grant_inactive`,
`not_attenuated`, `no_approval`, `approval_corrupt`, `approval_replayed`,
`approval_revoked`, `approval_expired`, mission/action mismatch, or
`authority_io_failure`). Consumption precedes the capability check, so a
request that fails a later budget or scope check has already burned its
one-shot approval — the failure direction is always closed. Worker-supplied
...
- one fixed-width parent-grant record (`parent.grant`) and one fixed-width
  ...
  written tmp-then-rename with `F_FULLFSYNC` on file and directory; a crash
  between approval consumption and the authorization event leaves the
  approval consumed (fail closed), never replayable.
